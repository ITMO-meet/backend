...
async def select_tags(payload: TagSelectionModel):
    ...
    tag_objects = await tags_collection.find({"name": {"$in": payload.tags}}).to_list(length=None)
...
async def add_profile_details(payload: ProfileDetailsModel):
    ...
    if update_result.modified_count == 0:
        raise HTTPException(status_code=404, detail="User not found or profile not updated")
